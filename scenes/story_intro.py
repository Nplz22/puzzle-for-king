...
class StoryIntro:
    def __init__(self, lines, bgm_path=None, previous_scene=None, next_scene=None, sfx_path=None, bg_image_path=None):
        self.lines = lines
        self.index = 0
    ...
    def handle_event(self, event):
        if event.type == pygame.KEYDOWN:
            if event.key in (pygame.K_RETURN, pygame.K_SPACE):
                ...
                if not self.options_scene:
                    self.options_scene = OptionsScene(previous_scene=self)
                return self.options_scene
        return None
